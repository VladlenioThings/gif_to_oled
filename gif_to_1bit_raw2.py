...
with open(OUTPUT_RAW, "wb") as fout:
    for frame in frames:
        ...
        if MODE == "fit":
            ...
            # если кадр уже помещается — просто центрируем
            if frame.width <= FRAME_WIDTH and frame.height <= FRAME_HEIGHT:
                ox = (FRAME_WIDTH - frame.width) // 2
                oy = (FRAME_HEIGHT - frame.height) // 2
                out_frame.paste(frame, (ox, oy))
            else:
                # кадр больше целевого размера — уменьшаем с сохранением пропорций
                tmp = frame.copy()
                tmp.thumbnail((FRAME_WIDTH, FRAME_HEIGHT), Image.Resampling.NEAREST)
                ox = (FRAME_WIDTH - tmp.width) // 2
                oy = (FRAME_HEIGHT - tmp.height) // 2
