...
with open(OUTPUT_RAW, "wb") as fout:
    for frame in frames:
        ...
        out_frame = Image.new("L", (FRAME_WIDTH, FRAME_HEIGHT), bg_color)
        ...
        if MODE == "fit":
            ...
            # ручная вставка каждого пикселя
            for y in range(new_h):
                for x in range(new_w):
                    out_frame.putpixel((ox + x, oy + y), tmp.getpixel((x, y)))
            ...
            # stretch
            tmp = frame.resize((FRAME_WIDTH, FRAME_HEIGHT), Image.Resampling.NEAREST)
            for y in range(FRAME_HEIGHT):
                for x in range(FRAME_WIDTH):
                    out_frame.putpixel((x, y), tmp.getpixel((x, y)))
